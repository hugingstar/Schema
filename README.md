...
flowchart TD
    %% Input Layer
    A[원본 메타데이터 CSV 파일<br>88.csv / R5points.csv] -->|1. 파일 입력| B(AutoMapping 클래스 호출)
    
    %% Initialization and Reading
    B -->|2. 컨텍스트 초기화| C[InferenceManager / DataModel]
    C -->|3. 파일 스트림 파싱| D[Dataset.loadDatasetFromFile]
    
    %% Tokenization
    D -->|4. 원시 문자열 텍스트 전달| E[SimpleTokenizer]
    E -->|CamelCase 변환 / 특수문자 제거| F[PointTokens 생성 및 정렬]
    
    %% Graph Construction
    F -->|5. 그래프 엔진 가동| G[PointGraph<br>토큰 폴더 계층 트리 DiGraph 구축]
    G -->|하위 노드 구조 대조| H[SimilarityGraph<br>닮은꼴 노드 클러스터링 & ConceptNode 생성]
    H -->|장비 명명 및 관계 조율| I[SemanticGraph<br>상속 및 포함 관계 최종 추론]
    
    %% Labeling and ML Scoring
    I -->|6. 그래프 전수 조사| J[InferenceManager.labelDataset]
    J -->|7. 피처 스코어링 대조| K[Features.py / PointClassifier.py<br>Jaro-Winkler 거리 채점]
    K -->|임계치 limit 이상 필터링| L[labeledDataset 캐시 적재]
    
    %% Output and Post-Processing
    L -->|8. 중간 가공물 출력| M[saveLabeledDataset<br>AutomapCandidate.csv 생성]
    M -->|9. 데이터 정제 엔진 가동| N[AutoMapping.LabelCleansing]
    N -->|최고 유사도 1:1 탐욕적 매칭| O[Pandas 데이터 프레임 배치 빌드]
    O -->|10. 마스터 리포트 출력| P[Automapping.csv 최종 저장완료]
